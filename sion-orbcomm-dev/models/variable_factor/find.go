...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]VariableFactor, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.VariableFactor.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableFactor.Find.close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.VariableFactor.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableFactor.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableFactor.Find.Scan: ", err)
			}
