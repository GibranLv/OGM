...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]Factor, error) {
	factors := []Factor{}
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Factor.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Factor.Find.Close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Factor.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Factor.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Factor.Find.Scan: ", err)
			}

		} else {
			factors = append(factors, orbcomm)
		}
	}
