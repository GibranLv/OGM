...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/charts/content.jsx");
/******/ })
/************************************************************************/
/******/ ({
...
var _itemVariable = __webpack_require__(/*! ./item-variable.jsx */ "./src/charts/item-variable.jsx");
  ...
  _createClass(Content, [{
    ...
    value: function componentDidMount() {
      ...
      (0, _async.parallel)({
        ...
        units: function units(fn) {
          self.getUnits(fn);
        },
        charts: function charts(fn) {
          self.getCharts(fn);
        }
      }, function (err, res) {
        if (err) {
        ...
        for (var i = 0; i < charts.length; i++) {
          ...
          for (var j = 0; j < variablesIn.length; j++) {
            var variable = variablesIn[j];
            if (chart.variable_id == variable.id) {
              charts[i].name = variable.name;
    ...
    key: 'getCharts',
    value: function getCharts(fn) {
      var self = this;

      var url = _constants2.default.URL_SERVER_CHARTS + '/list';
    ...
    value: function updateChart(json) {
      ...
      var xhr = $.ajax({
        url: _constants2.default.URL_SERVER_CHARTS,
      ...
      xhr.done(function (res, status, response) {
        if (response.status == _constants2.default.STATUS_OK) {
          self.updateItems(null, res.docs);
        } else if (response.status == _constants2.default.STATUS_ACCEPTED) {
          self.updateItems(res.message);
        }
      });
    ...
    key: 'updateItems',
    value: function updateItems(err, items) {
    ...
    value: function handleUpdate() {
      ...
      var fn = function fn(evt) {
        ...
        for (var i = 0; i < variables.length; i++) {
          ...
          var key = '#input-color-' + i;
          var inputColor = document.querySelector(key);
          if (inputColor) {
        ...
        if (variablesOut.length == 0) {
          var message = 'La lista no contiene ningún elemento';
          alert(message);
          return;
        }
    ...
    value: function handleInsertVariable() {
      ...
      var fn = function fn() {
        ...
        if (variableInsert) {
          var isNew = true;
          var _variables = self.state.variables;
          for (var _i = 0; _i < _variables.length; _i++) {
            var _variable = _variables[_i];
            if (_variable.variable_id == variableInsert.variable_id) {
              if (!variableInsert.is_custom) {
                isNew = false;
                break;
              }
            }
          ...
          if (isNew) {
            _variables.push(variableInsert);
            self.setState({ variables: _variables });
          }
        }
      };
    ...
    value: function handleInsertCustomVariable() {
      ...
      var fn = function fn() {
        ...
        if (variableInsert) {
          var isNew = true;
          var _variables2 = self.state.variables;
          for (var _i2 = 0; _i2 < _variables2.length; _i2++) {
            var _variable2 = _variables2[_i2];
            if (_variable2.variable_id == variableInsert.variable_id) {
              if (variableInsert.is_custom) {
                isNew = false;
                break;
              }
          ...
          if (isNew) {
            _variables2.push(variableInsert);
            self.setState({ variables: _variables2 });
          }
        }
      };
    ...
    value: function createItemVariable() {
      ...
      var fn = function fn(item, index) {
        ...
        return (0, _preact.h)(_itemVariable2.default, { key: index, item: item, onChangeUnit: self.handleChangeUnit() });
      };
    ...
    value: function render(props, state) {
      return (0, _preact.h)(
        ...
        (0, _preact.h)('div', { className: 'col-md-6' })
      );
    }
  }]);
...
var ItemVariable = function (_Component) {
  ...
  _createClass(ItemVariable, [{
    key: 'componentDidMount',
    value: function componentDidMount() {
      var variable = this.props.item;
      var units = variable.units_;
      if (units) {
        this.setState({ units: units }, function () {
          var index = variable.index;
          var unit_id = variable.unit_id;
          var color = variable.color;

          var keyUnit = '#input-unit-' + index;
          $(keyUnit).val(unit_id);

          var keyColor = '#input-color-' + index;
          $(keyColor).val(color);
        });
      }
    }
    ...
    value: function render(props, state) {
      ...
      var keyColor = 'input-color-' + variable.index;
      var keyUnit = 'input-unit-' + variable.index;
        ...
        (0, _preact.h)(
          ...
          (0, _preact.h)(
            ...
            (0, _preact.h)(
              'tr',
              null,
              (0, _preact.h)('input', { id: keyColor, type: 'text', className: 'form-control form-control-sm', placeholder: 'Color' })
            ),
            (0, _preact.h)(
              'tr',
              null,
              (0, _preact.h)(
                ...
                (0, _preact.h)(
                  'select',
                  { id: keyUnit, className: 'form-control form-control-sm', onChange: this.handleChangeUnit() },
                  (0, _preact.h)(
                    'option',
                    { value: '', selected: true },
                    'Unidad'
