...
func (m Model) Find(where map[string]interface{}) ([]Comment, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.CustomVariable.Comment.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.CustomVariable.Comment.Find.Close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.CustomVariable.Comment.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.CustomVariable.Comment.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.CustomVariable.Comment.Find.Scan: ", err)
			}
