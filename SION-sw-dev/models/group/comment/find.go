...
func (m Model) Find(where map[string]interface{}) ([]Comment, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Group.Comment.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Group.Comment.Find.Close: ", err)
			}
		}
	}(db)
	...
	query := "SELECT * FROM groups_comments"
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Group.Comment.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		err := r.Close()
		if err != nil {
			fmt.Println("Model.Group.Comment.Find.Rows.Close: ", err)
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if err != nil {
				fmt.Println("Model.Group.Comment.Find.Scan: ", err)
			}
