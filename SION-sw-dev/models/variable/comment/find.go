...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]Comment, error) {
	comments := []Comment{}
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Variable.Comment.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Variable.Comment.Find.Close: ", err)
			}
		}
	}(db)
	...
	query := "SELECT * FROM variables_comments"
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Variable.Comment.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Variable.Comment.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Variable.Comment.Find.Scan: ", err)
			}
