...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]VariableActiveAlarm, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.VariableActiveAlarm.Find.Open: ", err)
		}

		return variableActiveAlarms, err
	}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableActiveAlarm.Find.Close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.VariableActiveAlarm.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableActiveAlarm.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.VariableActiveAlarm.Find.Scan ", err)
			}
