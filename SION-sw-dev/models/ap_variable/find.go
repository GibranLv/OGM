...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]APVariable, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.APVariable.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.APVariable.Find.Close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.APVariable.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.APVariable.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.APVariable.Find.Scan: ", err)
			}
