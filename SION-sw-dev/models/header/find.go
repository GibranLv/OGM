...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]Header, error) {
	headers := []Header{}
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Header.Find.Open: ", err)
		}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Header.Find.close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.Header.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Header.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.Header.Find.Scan: ", err)
			}
