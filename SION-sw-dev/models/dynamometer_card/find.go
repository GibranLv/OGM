...
// Find ...
func (m Model) Find(where map[string]interface{}) ([]DynamometerCard, error) {
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.DynamometerCard.Find.Open: ", err)
		}

		return dynamometerCards, err
	}
	...
	defer func(r *sql.DB) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.DynamometerCard.Find.Close: ", err)
			}
		}
	}(db)
	...
	if err != nil {
		if m.Debug {
			fmt.Println("Model.DynamometerCard.Find.Query: ", err)
		}
	...
	defer func(r *sql.Rows) {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.DynamometerCard.Find.Rows.Close: ", err)
			}
		}
	}(rows)
	...
	for rows.Next() {
		...
		if err != nil {
			if m.Debug {
				fmt.Println("Model.DynamometerCard.Find.Scan: ", err)
			}
