...
class Content extends Component {
  ...
  getGPSDevicesForSearch(value) {
    let self = this;

    let url = `${constants.URL_SERVER_GPS_DEVICES}/list?search=${value}`;
  ...
  getGPSDevice() {
    ...
    let fn = (json) => {
      let xhr = $.ajax({
        url: `${constants.URL_SERVER_GPS_DEVICES}/${json.id}`,
        type: constants.METHOD_GET,
        dataType: constants.JSON
      });
  ...
  addGPSDevice() {
    ...
    let fn = (json) => {
      let xhr = $.ajax({
        url: constants.URL_SERVER_GPS_DEVICES,
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_CREATED) {
          let message = 'El dispositivo GPS se creó correctamente';
  ...
  updateGPSDevice() {
    ...
    let fn = (json, id) => {
      ...
      let xhr = $.ajax({
        url: `${constants.URL_SERVER_GPS_DEVICES}/${id}`,
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_OK) {
          let message = 'El dispositivo GPS se actualizo correctamente';
          Materialize.toast(message, 2500);
  ...
  deleteGPSDevice() {
    ...
    let fn = (json) => {
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_OK) {
          let message = 'El dispositivo GPS se elimino correctamente';
          Materialize.toast(message, 2500);
      ...
      if (evt.which == ENTER) {
        ...
        if (search) {
          if (search !== '') {
            ...
            self.findGPSDevicesForSearch(search);
          }
        }
      }
    };
  ...
  findGPSDevicesForSearch(search) {
    let items = this.state.items;
    ...
    if (isArray(items)) {
      ...
      let res = items.filter((item) => {
        ...
        let keys = ['imei', 'phone_number', 'status'];
  ...
  render(props, state) {
    ...
    let createForm = <CreateForm onCreate={self.addGPSDevice()} onBack={self.handleBack()} />
    let updateForm = <UpdateForm item={state.item} onUpdate={self.updateGPSDevice()} onBack={self.handleBack()} />
      ...
      return (
        <div>
          ...
          <Table items={state.items_}
                 ...
                 page={state.page_}
                 onGet={self.getGPSDevice()}
                 onDelete={self.deleteGPSDevice()}
