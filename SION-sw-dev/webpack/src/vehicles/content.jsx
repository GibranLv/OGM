...
class Content extends Component {
  ...
  getVehiclesForSearch(value) {
    let self = this;

    let url = `${constants.URL_SERVER_VEHICLES}/list?search=value`;
  ...
  getVehicle() {
    ...
    let fn = (json) => {
      let xhr = $.ajax({
        url: `${constants.URL_SERVER_VEHICLES}/${json.id}`,
        type: constants.METHOD_GET,
        dataType: constants.JSON
      });
  ...
  addVehicle() {
    ...
    let fn = (json) => {
      let xhr = $.ajax({
        url: constants.URL_SERVER_VEHICLES,
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_CREATED) {
          let message = 'El vehiculo se creó correctamente';
  ...
  updateVehicle() {
    ...
    let fn = (json, id) => {
      ...
      let xhr = $.ajax({
        url: `${constants.URL_SERVER_VEHICLES}/${id}`,
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_OK) {
          let message = 'El vehiculo se actualizo correctamente';
          Materialize.toast(message, 2500);
  ...
  deleteVehicle() {
    ...
    let fn = (json) => {
      ...
      xhr.done((res, status, response) => {
        if (response.status == constants.STATUS_OK) {
          let message = 'El vehiculo se elimino correctamente';
          Materialize.toast(message, 2500);
      ...
      if (evt.which == ENTER) {
        ...
        if (search) {
          if (search !== '') {
            ...
            self.findVehiclesForSearch(search);
          }
        }
      }
    };
  ...
  getViewItem(item) {
    let gps_device = item.gps_device;
    if (gps_device == '') {
      gps_device = 'N/A';
    }
  ...
  findVehiclesForSearch(search) {
    ...
    if (isArray(items)) {
      ...
      let res = items.filter((item) => {
        ...
        let keys = ['gps_device', 'alias', 'license_plates', 'description', 'responsible', 'note', 'status'];
  ...
  render(props, state) {
    ...
    let createForm = <CreateForm onCreate={self.addVehicle()} onBack={self.handleBack()} />
    let updateForm = <UpdateForm item={state.item} onUpdate={self.updateVehicle()} onBack={self.handleBack()} />
      ...
      return (
        <div>
          ...
          <Table items={state.items_}
                 ...
                 page={state.page_}
                 onGet={self.getVehicle()}
                 onDelete={self.deleteVehicle()}
