...
class LocationModule extends Component {
	...
	componentDidMount() {
		...
		loadJS(
			'https://maps.googleapis.com/maps/api/js?key=AIzaSyDQ7lfWMFuJfAp7eXWVYJMZ69t4tX8-ZU8&callback=initMap'
		)
	...
	updateSizeMap() {
			const win = $(window)
			const wHeight = win.height()
			const sub = $('#TTx-nav-top').height() + $('.menu_top').height()

			const nHeight = wHeight - sub

			$('#map').css('height', `${nHeight}px`)
	}
	...
	render(props, state) {
		...
		return (
			<div>
				<Header
					o={o}
					module={constants.LOCATION_MODULE}
