...
const wsURL = `ws://${window.URLWS}/ws`
const wsaURL = `ws://${window.URLWSA}/ws`
...
class MatricesModule extends Component {
	constructor(props) {
		super(props)
	...
	componentDidMount() {
		...
		window.$('.modal').modal({ dismissible: false })
			...
			parallel(
				{
					...
					log_alarms: (fn) => {
						if (window.SYSTEM_HOST === 'sepec.technotex.com') {
							fn()
							return
						}
				...
				(err, res) => {
					...
					footer_variables = self.orderByOS(
						footer_variables,
						'position',
						true
					)
	...
	setUpCharts() {
		window.Highcharts.createElement(
			'link',
			{
		...
		if (window.CHART_THEME === constants.DARK_THEME) {
	...
	getTokenWS() {
		const url = `${constants.URL_SERVER_USERS}/tokens?${constants.ACCESS_TOKEN_WS}=true`

		const xhr = window.$.ajax({
			url: url,
		...
		xhr.done((res, _, response) => {
			if (response.status == constants.STATUS_OK) {
				...
				const token_ws = doc.access_token_ws
				if (token_ws)
					window.sessionStorage.setItem(
						constants.ACCESS_TOKEN_WS,
						token_ws
					)
	...
	getTokenWSA() {
		const url = `${constants.URL_SERVER_USERS}/tokens?${constants.ACCESS_TOKEN_WSA}=true`
	...
	getConfiguration(fn) {
		...
		const url = `${constants.URL_SERVER_USERS}/sounds/matrix`
	...
	getMatrices() {
		...
		const url = `${constants.URL_SERVER_MATRICES}/list?with_structure=true&with_structure_json=false`
		...
		xhr.done((res, _, response) => {
			if (response.status == constants.STATUS_OK) {
				self.setState({ matrices_: res.docs }, () => {
					...
					if (!isArray(self.state.matrices_)) return
					...
					if (self.state.matrices_.length > 0) {
						...
						if (isNumber(window.MatrixID)) {
							if (window.MatrixID > 0) {
								let matrices_ = self.state.matrices_
								if (!isArray(matrices_)) matrices_ = []

								const matrices = matrices_.filter((matrix) => {
									const { id } = matrix

									return window.MatrixID === id
								})

								const structures = matrices.map(
								...
								const matrices = matrices_.filter(
									(_, index) => {
										return index === 0
									}
								)
							...
							try {
								matricesID = JSON.parse(sJSON)
							} catch (e) {
								console.log(`MatrixID isn't Array in JSON `, e)
							}
							...
							const matrices = matrices_.filter((matrix) => {
								...
								return matricesID.includes(id)
							})

							const structures = matrices.map(
								(matrix) => matrix.structure
							)
							const nameMatrices = matrices
				...
				variables: (fn) => {
					...
					xhr.done((res, _, response) => {
						if (response.status == constants.STATUS_OK) {
							...
							response.status == constants.STATUS_ACCEPTED
						) {
							fn(res.message)
						}
					})
					...
					for (let i = 0; i < variables.length; i++) {
						...
						for (let j = 0; j < variablesIn.length; j++) {
							...
							if (variable.id == variableIn.id) {
								if (
									variable.is_custom == variableIn.is_custom
								) {
									isNew = false
									break
								}
	...
	updateVariableCommentInMatrix(content) {
		...
		if (isArray(content)) {
			if (content.length > 0) {
				...
				self.setState({ structures: structures }, () => {
					window.$('.tooltipped').tooltip({ delay: 20 })
				})
			}
		}
	}
					...
					if (v.id == variable_id) {
						if (v.is_custom == is_custom) {
							...
							if (alarm_id) {
								...
								groups[i].variables[j].alarm_setpoint =
									alarm_setpoint
							} else {
	...
	updateGroupComment(groups, group_id, comment) {
		for (let i = 0; i < groups.length; i++) {
			...
			if (g.sons) {
				groups[i].sons = this.updateGroupComment(
					g.sons,
					group_id,
					comment
				)
				...
				if (value != '') {
					...
					if (comment.variable_id) {
						...
						if (comment.is_custom) {
							self.updateCommentCustomVariable(
								comment.variable_id,
								o
							)
						} else {
					...
					inputComment.value = ''
					window.$('#comentarios_macro').modal('close')
				}
			}
		}
					...
					if (active_variable.is_custom == is_custom) {
						...
						self.setState({ structures: structures }, () => {
							...
							if (matrices) {
								let matrix = matrices[mi]
								if (matrix) {
									let active_vars = clone(
										self.state.active_vars
									)
									let variables = self.getVariables(
										matrix.structure
									)
			...
			for (let i = 0; i < size; i++) {
				...
				if (logOne.id === id) {
					...
					if (!checked) {
						...
						window.$('#comentarios_log_alarm').modal('open')
						window.$('#input-comment-log-alarm').val(logOne.comment)
					}
	...
	handleCommentLogAlarm() {
		...
		let fn = (evt) => {
			...
			if (window.SYSTEM_HOST === 'sepec.technotex.com') return

			let comment = window.$('#input-comment-log-alarm').val()
	...
	handleChangeMatrix() {
		...
		const fn = (m, s) => {
			...
			if (isNumber(m)) {
				...
				const structures = matrices.map((matrix) => matrix.structure)
	...
	handleRemoveNotification() {
		...
		const fn = (id) => {
			const notifications = self.state.notifications_

			if (isArray(notifications)) {
				self.updateEventAsSeen(id)
				const filtered = notifications.filter((n) => n.id !== id)
				self.setState({ notifications_: filtered })
			}
		/* 			
	...
	createItemFV() {
		const fn = (variable, index) => {
			...
			if (variable_value === '0' || variable_value === 0) {
				variable_value = (
					<span style='color: #F2ED0A !important;'>
						{variable_value}
					</span>
				)
			}
						...
						<div className='row'>
							<div className='col s12 m12'>
								...
								>
									...
									<tbody>
										{log_alarms.map(self.createLogAlarm())}
									</tbody>
								</table>
							</div>
						</div>
			...
			<div>
				...
				<section className={contentClass}>
					<div className='matriz_clasica'>
						<div className='row'>
							{structures.map(this.createMatrix())}
						</div>
					</div>
				...
				<div id='comentarios_macro' className='modal modal_sesion'>
					<div className='modal-content'>
						<i className='material-icons animated fadeInDown'>
							announcement
						</i>
				...
				<div id='paro_remoto' className='modal modal_sesion'>
					<div className='modal-content'>
						...
						<div className='modal_box animated fadeIn'>
							...
							<br />
							<h5>
								¿Está seguro de que quieres realizar esta
								acción?
							</h5>
							<br />
							<a
