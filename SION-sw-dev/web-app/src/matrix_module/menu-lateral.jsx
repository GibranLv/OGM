...
class MenuLateral extends Component {
	...
	handleChangeMatrix(value) {
		...
		let fn = (evt) => {
			...
			let o = self.props.o
			if (o) {
				const f = self.props.onChangeMatrix
				if (!f) return
          ...
          return
				...
				const matrix = matrices.find((matrix) => matrix.id === value)
				if (!matrix) return

				f(matrix, matrix.structure, 0, 0)
			}
		}
			...
			return (
				<li key={key}>
					<span style='color: #888; margin-left: 20px;'>
						<a href='#' onClick={self.handleChangeMatrix(item.id)}>
							{item.name}
						</a>
					</span>
				</li>
			...
			return (
				<li key={key}>
					<div className='collapsible-header'>
						...
						<div
							className='col s10'
							onClick={this.handleItemGroup(mi, item)}
						>
							<span>{item.name}</span>
						</div>
					</div>
				...
				<li>
					<div className='collapsible-header'>
						<i
							...
							onClick={self.handleRestoreMatrix(index)}
						>
							developer_board
						</i>
						{matrix.name}
					...
					<div className='collapsible-body'>
						<ul
							...
							data-collapsible='expandable'
						>
							{matrix.structure.map(self.createItemGroup(index))}
						</ul>
					</div>
				</li>
			)
