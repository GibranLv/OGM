...
class Chart extends Component {
								...
								self.setState({ isOk: false }, () => {
									...
									if (chartKey === '24') {
										const { dataMin, dataMax } =
											self.chartOne.xAxis[0].getExtremes()
									...
									for (let j = 0; j < content.length; j++) {
										...
										if (serie.variable_id === id) {
											if (serie.is_custom === isCustom) {
												...
												let hasConversion =
													isString(expression) && isString(display)

												if (hasConversion && isNumber(value) && !isNaN(value)) {
													let nExpression = self.replaceAll(
														expression,
														'${value}',
														value
													)
												...
												if (self.chartOne.series[serie.position]) {
													...
													self.chartOne.series[serie.position].addPoint(
														point,
														true,
														shifted
													)
		...
		let config = {
			...
			chart: {
				...
				events: {
					click: function (e) {
						let min = document
							.querySelector(`#${INPUT_SPAN_MIN}-${chartKey}`)
							.value.trim()
						min = parseFloat(min)

						let max = document
							.querySelector(`#${INPUT_SPAN_MAX}-${chartKey}`)
							.value.trim()
						max = parseFloat(max)
					...
					point: {
						events: {
							click: function (e) {
								if (e.point) {
									...
									if (e.point.options) {
										...
										if (o.chart_event_id) {
											...
											if (id) {
												self.props.chartEmitter.emit(
													constants.EVENT_OPEN_EVENT_CHART,
													id
												)
											}
								...
								let keyInputNewEvent = `input-new-event-${chartKey}`
								let nEvent = document.querySelector(
									`#${keyInputNewEvent}`
								).checked
								if (!nEvent) return
								...
								for (let i = 0; i < variables.length; i++) {
									...
									if (name === variable_name) {
										...
										if (point.id === 0) {
											let message =
												'Solo es posible agregar eventos en periodos menores a 15 días'
	...
	initializePickers() {
		...
		$('#' + keyContentDateOf).append(
			'<input type="text" id="' +
				keyDateOf +
				'" class="datepicker" placeholder="De:"  value="' +
				date_of +
				'" />'
		)

		$('#' + keyContentDateTo).html('')
		$('#' + keyContentDateTo).append(
			'<input type="text" id="' +
				keyDateTo +
				'" class="datepicker" placeholder="A:"  value="' +
				date_to +
				'" />'
		)
	...
	handleResetZoom() {
		...
		let fn = (evt) => {
			...
			if (self.chartOne) {
				...
				for (let i = 0; i < sizeX; i++) {
					self.chartOne.xAxis[i].setExtremes(
						self.extremes.x.min,
						self.extremes.x.max
					)
				}
					...
					if (variable.is_custom === is_custom) {
						if (variable.id === id) {
							if (!variable.in_variables) {
								...
								self.setState(
									{ variables: variablesIn, variables_: variables },
									() => {
										$(keyInputVariable).val('')
										...
										$(keyInputDevice).off('change', self.handleChangeDevice())
										$(keyInputVariable).off(
											'change',
											self.handleChangeVariable()
										)
		...
		let fn = (item, index) => {
			return (
				<div className='sion-chip' key={index}>
					...
					<i
						className='close material-icons'
						onClick={self.handleRemoveItem(index)}
					>
						close
					</i>
				</div>
				...
				<div id='menugrafica' className='menu_grafica'>
							...
							<div className='tools_vars'>
								<div className='variable'>
									...
									>
										...
										{state.devices_
											.sort(this.orderByDevice())
											.map(this.createOptDevices())}
									</select>
								</div>
								...
								<div className='variable'>
									...
									>
										...
										{state.variables_
											.sort(this.orderByVariable())
											.map(this.createOptVariable())}
									</select>
								</div>
						...
						<div className='col s12 m2'>
							...
							<div className='btn_actions'>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_24)}>
										24 <span>Horas</span>
									</a>
								</div>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_48)}>
										48 <span>Horas</span>
									</a>
								</div>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_07)}>
										07 <span>Días</span>
									</a>
								</div>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_30)}>
										30 <span>Días</span>
									</a>
								</div>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_06)}>
										06 <span>Meses</span>
									</a>
								</div>
								<div className='btn_last hoverable'>
									<a href='#' onClick={this.handleLastBtn(LAST_12)}>
										12 <span>Meses</span>
									</a>
								</div>
							</div>
							...
							<div className='realtime_maxmin'>
								...
								<div className='col s6 m4'>
									<div className='margin_top'>
										<button
											className='btn btn_ttx_flat btn-ttx-graph'
											id='btn-graph'
					...
					<div className='row'>
						<div
							...
							style='background: #313131; height: 570px;'
						>
							<div
								id={keyChartContent}
								style='height: 550px; width: 100%; margin: 10px auto'
							></div>
						</div>
					</div>
					...
					<div
						id={keyBtnResetZoom}
						style='displa: none; position: absolute; width: 100px;'
					>
						<button
							className='btn btn_ttx_flat'
