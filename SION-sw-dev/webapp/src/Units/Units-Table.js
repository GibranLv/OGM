...
class UnitsRowTable extends React.Component {
  ...
  handleDelete() {
    ...
    let fn = (evt) => {
      ...
      let message = `¿Desea eliminar la unidad: ${name}?`;
...
class UnitsTable extends React.Component {
  ...
  createRow() {
    ...
    let fn = (item) => {
      return <UnitsRowTable key={item.id} row={item} onGet={self.handleGet()} onDelete={self.handleDelete()} />;
    };
  ...
  render() {
    ...
    if (!rows) {
      rows = <tr><td className="center" colSpan="5">Sin unidades registradas</td></tr>;
    }
