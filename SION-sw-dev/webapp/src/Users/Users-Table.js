...
class UsersRowTable extends React.Component {
  ...
  handleDelete() {
    ...
    let fn = () => {
      ...
      let message = `¿Desea eliminar el grupo: ${name}?`;
  ...
  render() {
    ...
    return (
      <tr>
        ...
        <td>{row.name}</td>
        <td>
          <a title="Editar" className="btn-floating blue" href="#options" onClick={this.handleOptions()}>
...
class UsersTable extends React.Component {
  ...
  createRow() {
    ...
    let fn = (item, index) => {
      return <UsersRowTable key={index} row={item} onGet={self.handleGet()} onDelete={self.handleDelete()} onOptions={self.handleOptions()} />;
    };
  ...
  render() {
    ...
    if (!rows) {
      rows = <tr><td className="center" colSpan="6">Sin usuarios registrados</td></tr>;
    }
