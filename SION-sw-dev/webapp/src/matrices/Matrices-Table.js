...
class MatricesRowTable extends React.Component {
  ...
  handleDelete() {
    ...
    let fn = (evt) => {
      ...
      let message = `¿Desea eliminar la matriz: ${name}?`;
...
class MatricesTable extends React.Component {
  ...
  createRow() {
    ...
    let fn = (item) => {
      return <MatricesRowTable key={item.id} row={item} onGet={self.handleGet()} onDelete={self.handleDelete()} />;
    };
  ...
  render() {
    ...
    if (!rows) {
      rows = <tr><td className="center" colSpan="3">Sin matrices registradas</td></tr>;
    }
