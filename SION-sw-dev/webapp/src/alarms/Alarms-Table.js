...
class AlarmsRowTable extends Component {
  ...
  render() {
    ...
    let sName = { color: `${row.color}`, fontWeight: 'bold' };
...
class AlarmsTable extends Component {
  ...
  createRow() {
    ...
    let fn = (item, index) => {
      return <AlarmsRowTable key={index} row={item} onGet={self.handleGet()} onDelete={self.handleDelete()} onOptions={self.handleOptions()} />;
    };
