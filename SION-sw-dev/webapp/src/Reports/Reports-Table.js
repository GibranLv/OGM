...
class ReportsRowTable extends Component {
  ...
  handleDelete() {
    ...
    let fn = () => {
      ...
      let message = `¿Desea eliminar el reporte: ${name}?`;
...
class ReportsTable extends Component {
  ...
  createRow() {
    ...
    let fn = (item) => {
      return <ReportsRowTable key={item.id} row={item} onGet={self.handleGet()} onDelete={self.handleDelete()} />;
    };
  ...
  render() {
    ...
    if (!rows) {
      rows = <tr><td className="center" colSpan="3">Sin reportes registrados</td></tr>;
    }
